...
public class ModBlockStateProvider extends BlockStateProvider {
    ...
    private void generateLatexCollectorBlockState(ModelFile emptyModel, ModelFile fillingModel) {
        getMultipartBuilder(ModBlocks.LATEX_COLLECTOR.get())
                ...
                .part().modelFile(fillingModel).addModel().condition(LatexCollectorBlock.HORIZONTAL_FACING, Direction.NORTH)
                .condition(LatexCollectorBlock.FILL_STATUS, LatexCollectorBlock.FillStatus.FILLING).end()
                .part().modelFile(fillingModel).rotationY(90).addModel().condition(LatexCollectorBlock.HORIZONTAL_FACING, Direction.EAST)
                .condition(LatexCollectorBlock.FILL_STATUS, LatexCollectorBlock.FillStatus.FILLING).end()
                .part().modelFile(fillingModel).rotationY(180).addModel().condition(LatexCollectorBlock.HORIZONTAL_FACING, Direction.SOUTH)
                .condition(LatexCollectorBlock.FILL_STATUS, LatexCollectorBlock.FillStatus.FILLING).end()
                .part().modelFile(fillingModel).rotationY(270).addModel().condition(LatexCollectorBlock.HORIZONTAL_FACING, Direction.WEST)
                .condition(LatexCollectorBlock.FILL_STATUS, LatexCollectorBlock.FillStatus.FILLING);
    }
    ...
    private ModelFile generateCubeMachineModel(String name) {
        return models().getBuilder(name)
                .parent(new ExistingModelFile(Identifier.fromNamespaceAndPath("minecraft", "block/cube"), exFileHelper))
